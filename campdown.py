...
if __name__ == "__main__":
    try:
        ...
        try:
            bandcamp_band = str(r).split("var BandData = {", 1)[1].split("}")[
        ...
        if str(r).find("track_list") == -1:
            ...
            bandcamp_album = re.sub(
                '[:*?<>|]', '', str(r).split('<meta name="Description" content=')[1].split(" by ")[0][3:])

            if not os.path.exists(outputfolder + "/" + bandcamp_album + "/"):
                print('\nCreated album folder in %s' %
                      (outputfolder + bandcamp_album + "/"))
                ...
                if position != -1:
                    ...
                    if trackname != "":
                        ...
                        track_r = requests.get(
                            bandcamp_base_url + "/track" + trackname).content
                        rawinfo = ("{" + (str(track_r).split("trackinfo: [{", 1)[1].split("}]")[
                                   0]) + "} ").replace('\\n', ' ').replace('\\', ' ')
                        trackinfo = json.loads(rawinfo)
        ...
        for i in range(0, len(bandcamp_queue)):
            ...
            if title.find(" - ") != -1:
                ...
                if bandcamp_isAlbum:
                    ...
                    title = partialtitle[
                        0] + " - " + bandcamp_album + " - " + partialtitle[1]
            else:
                if bandcamp_isAlbum:
                    title = bandcamp_band + " - " + bandcamp_album + \
                        " - " + str(i + 1) + " " + title
                else:
                    title = bandcamp_band + " - " + \
                        bandcamp_album + " - " + title
                ...
                try:
                    with open(outputfolder + "/" + re.sub('[\\/:*?<>|]', '', title) + ".mp3", "wb") as f:
                        ...
                        if total_length is None:
                            ...
                            cleaned_length = int(
                                (total_length * 100) / pow(1024, 2)) / 100
                            ...
                            try:
                                ...
                                print(
                                    "An error occured while downloading - skipping track")
